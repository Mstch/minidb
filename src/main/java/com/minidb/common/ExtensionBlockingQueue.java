...
public class ExtensionBlockingQueue<E> extends AbstractQueue<E>
    ...
    public ExtensionBlockingQueue(int capacity) {
        this(capacity, false);
    }


    public ExtensionBlockingQueue(int capacity, boolean fair) {
        if (capacity <= 0)
    ...
    private class Itr implements Iterator<E> {
        ...
        Itr() {
            lastRet = NONE;
            final ReentrantLock lock = ExtensionBlockingQueue.this.lock;
            lock.lock();
            try {
                if (count == 0) {
                    cursor = NONE;
                    ...
                    final int takeIndex = ExtensionBlockingQueue.this.takeIndex;
    ...
    public E peekSync() throws InterruptedException {
        final ReentrantLock lock = this.lock;
        lock.lockInterruptibly();
        try {
            while (count == 0)
